# copied from original at $(PSPDEV)/lib/build.mak
# so that I can add some missing things
...
$(PSP_EBOOT_SFO): 
	mksfoex -d MEMSIZE=1 -s DISC_ID='$(PSP_EBOOT_DISC_ID)' '$(PSP_EBOOT_TITLE)' $@
...
clean: 
	-rm -f $(FINAL_TARGET) $(EXTRA_CLEAN) $(OBJS) $(PSP_EBOOT_SFO) $(PSP_EBOOT) $(EXTRA_TARGETS) common/*.o

rebuild: clean all

run:
	mkdir -p ~/ppsspp/$(TARGET) && cp EBOOT.PBP ~/ppsspp/$(TARGET)
	ppsspp-emu.ppsspp-sdl --escape-exit ~/ppsspp/$(TARGET)/EBOOT.PBP

runwin:
	"/mnt/c/program files/PPSSPP/PPSSPPWindows64.exe" --escape-exit ./EBOOT.PBP

deploy:
	mkdir -p /media/mohasi/disk/PSP/GAME/Test && cp EBOOT.PBP /media/mohasi/disk/PSP/GAME/Test